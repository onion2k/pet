...
import { BLAZE, GLOOP, GRUMP, LUMEN, MOCHI, VERDANT } from './models-adult'
...
export interface Branch {
  ...
  score(m: Metrics, pet: PetState): number
}
...
export function chooseBranch(pet: PetState, m: Metrics): Branch | null {
  const { branches } = speciesOf(pet.speciesId)
  if (branches.length === 0) return null
  return branches.reduce((best, b) => (b.score(m, pet) > best.score(m, pet) ? b : best))
}
